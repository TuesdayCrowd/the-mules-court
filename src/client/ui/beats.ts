...
import { dealDelayMs, motionPlan } from '../store/motion';
import { dealPath } from '../layout/dealPath';
...
export interface BeatContext {
    ...
    readonly rect?: Rect;
    /**
     * Where the beat starts, for the beats that travel.
     *
     * Only the deal has two ends: `fromRect` is the deck, `rect` the hand slot
     * or seat chip the card lands on. Every other beat happens in one place and
     * leaves this absent, which is why it is a second optional field rather
     * than a rename of `rect` — a rename would have every existing caller
     * saying "from" when it means "at".
     */
    readonly fromRect?: Rect;
    /**
     * This card position in a simultaneous deal, for the stagger.
     *
     * The delay itself comes from `dealDelayMs` in `store/motion.ts`, which is
     * where every duration in this file comes from. Absent means "the first
     * card", which is also the honest answer for a lone draw.
     */
...
/**
 * Decelerate — an arrival, from the project own named curve set (see the
 * easing-and-choreography skill). A dealt card leaves the deck fast and settles
 * into its slot; easing it any other way is what makes motion read as floaty.
 */
const EASE_DECELERATE = 'cubic-bezier(0, 0, 0, 1)';
...
/**
 * A card travelling from the deck into a hand.
 *
 * **It arcs, and the arc is the point.** A straight `translate()` between two
 * rects reads as a div moving; a card that banks into its curve reads as
 * *dealt*. That comes from CSS motion path — `offset-path` with an
 * `offset-distance` animated 0% → 100% — which also gives `offset-rotate: auto`
 * for free, so the card turns to face along the tangent with no per-frame JS.
 * `path()` rather than `shape()`: the former has been baseline since 2022,
 * the latter is not in Firefox.
 *
 * **The arc goes on a wrapper and the card scales inside it.** Motion path
 * occupies the transform slot — the offset transform and the `transform`
 * property compose, and writing both onto one element is how one of them
 * silently wins. Two elements, one job each.
 *
 * The path itself is generated by `dealPath` in `layout/`, pure and Node-tested,
 * because this table is responsive and a literal path string would be correct
 * at exactly one viewport size.
 */
async function deal(step: MotionStep, ctx: BeatContext, layer: HTMLElement, viewportRect: () => Rect): Promise<void> {
    // A deal with no art has nothing to move. `main.ts` supplies the card face
    // for the viewer own draw and the card BACK for an opponent — interface
    // rule 4, and the reason `card-drawn` carries no identity for another seat.
    if (ctx.portraitKey === undefined) return;

    const to = ctx.rect ?? viewportRect();
    const from = ctx.fromRect ?? to;

    // The wrapper box IS the deck rect: `path()` coordinates resolve against
    // the element own box, which is what `dealPath` generates against.
    const wrapper = baseEl('div');
    wrapper.style.left = px(from.x);
    wrapper.style.top = px(from.y);
    wrapper.style.width = px(from.w);
    wrapper.style.height = px(from.h);
    wrapper.style.setProperty('offset-path', `path('${dealPath(from, to)}')`);
    wrapper.style.setProperty('offset-anchor', 'center');
    wrapper.style.setProperty('offset-rotate', 'auto');
    wrapper.style.setProperty('offset-distance', '0%');
    layer.appendChild(wrapper);

    const card = baseEl('img');
    card.src = ctx.portraitKey;
    card.alt = '';
    card.style.left = '0';
    card.style.top = '0';
    ...
    // The card grows from deck size to the size of the slot it lands in.
    // Guarded, because a zero-width deck rect would scale by Infinity and the
    // card would simply vanish rather than fail visibly.
    const growth = from.w > 0 ? to.w / from.w : 1;
    const delay = dealDelayMs(ctx.staggerIndex ?? 0);
    const options: KeyframeAnimationOptions = { duration: step.durationMs, delay, easing: EASE_DECELERATE, fill: 'forwards' };

    try {
        await Promise.all([
            finished(wrapper.animate([{ offsetDistance: '0%' }, { offsetDistance: '100%' }], options)),
            finished(
                card.animate([{ transform: 'scale(1)', opacity: 0.6 }, { transform: `scale(${growth})`, opacity: 1 }], options)
            )
        ]);
    } finally {
        // Both elements go on every path, not just the happy one — the card is
        // a child of the wrapper, so removing the wrapper takes both.
        wrapper.remove();
    }
}
...
export function createBeatRunner(layer: HTMLElement, deps: BeatRunnerDeps): BeatRunner {
    ...
    async function runStep(step: MotionStep, ctx: BeatContext): Promise<void> {
        switch (step.kind) {
            ...
            case 'deal':
                return deal(step, ctx, layer, viewportRect);
            case 'ripple':
